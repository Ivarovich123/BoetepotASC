...
export default function EditReason({ params }: { params: { id: string } }) {
  const router = useRouter()
  const [loading, setLoading] = useState(true)
  const [submitting, setSubmitting] = useState(false)
  const [error, setError] = useState<string | null>(null)
  const [formData, setFormData] = useState<Partial<Reason>>({})
  ...
  const fetchReasonData = async () => {
    ...
    setError(null)
    try {
      const reasonId = Number(params.id)
      if (isNaN(reasonId)) {
        throw new Error('Ongeldig reden ID.')
      }

      const { data, error: fetchError, status } = await supabase
        .from('reasons')
        .select('id, description') // Removed amount
        .eq('id', reasonId)
        .single()

      if (fetchError) {
        if (status === 406) { // Not found
          throw new Error('Boetereden niet gevonden.')
        } else {
          throw fetchError
        }
      }
      ...
      console.error('Fout bij ophalen redengegevens:', error)
      setError(error.message || 'Kon de gegevens voor deze reden niet laden.')
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    if (!formData.id || !formData.description?.trim()) {
    ...
    try {
      const updatedReasonData = {
        ...
        // amount: amountValue // Removed
      };

      const { error: updateError } = await supabase
        .from('reasons')
        .update(updatedReasonData)
        .eq('id', formData.id)

      if (updateError) throw updateError
      router.push('/admin/reasons') // Redirect on success
    } catch (error: any) {
      console.error('Fout bij bijwerken reden:', error)
      // Check for unique constraint violation
      if (error.message?.includes('duplicate key value violates unique constraint')) {
          setError('Deze omschrijving bestaat al.');
      } else {
          setError(error.message || 'Kon de reden niet bijwerken.');
      }
    } finally {
      setSubmitting(false)
    }
      ...
      <div className="min-h-screen bg-gradient-to-br from-primary-50 to-secondary-100">
        ...
        <header className="bg-white shadow-soft">
          <div className="container mx-auto px-4 py-4 flex justify-between items-center">
            <h1 className="text-2xl font-bold text-primary-600">Boete Reden Bewerken</h1>
            <Link
              href="/admin/reasons"
          ...
          <div className="max-w-2xl mx-auto">
             {loading ? (
               <div className="bg-white rounded-xl shadow-soft p-12 text-center">
                 ...
                 <p className="text-secondary-600">Redengegevens worden geladen...</p>
               </div>
             ) : error && !formData.id ? (
              ...
              <div className="bg-white rounded-xl shadow-soft">
                <form onSubmit={handleSubmit} className="p-6 md:p-8 space-y-6">
                  <h2 className="text-xl font-semibold text-secondary-800 mb-4">Bewerk Reden Details</h2>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      type="text"
                      required
                      placeholder="Korte omschrijving van de boete"
